...
export function logToCurl(opts: HttpRequestConfig | HttpRequestErrorConfig) {
  ...
  const baseUrl = `${opts.response?.config?.baseURL || ''}${opts.url}`;
  const headers = {
  ...
  let curl = `curl -X ${(method || 'POST').toUpperCase()} '${url.toString()}'`;
  ...
  curl += Object.entries(headers).reduce(
    (curlHeaders, [key, value]) => `${curlHeaders} -H '${key}: ${value}'`,
    '',
  );
