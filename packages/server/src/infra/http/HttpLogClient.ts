...
const jsonStringify = (display: boolean, body: object) => {
  return body && display ? JSON.stringify(body) : '';
};
...
export class HttpLogClient {
  ...
  protected formatLog(options: HttpRequestConfig | HttpRequestErrorConfig, verbose: boolean) {
    ...
    return {
      ...
      callee_response_body:
        full && !options.responseType && data != undefined ? jsonStringify(full, data) : undefined,
