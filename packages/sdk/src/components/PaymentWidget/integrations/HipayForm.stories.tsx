...
export const LoadingIframeTest: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "Test complet du chargement : vérifie d'abord l'état de chargement avec animation, puis attend que le SDK Hipay charge et vérifie la création des 4 iframes hébergées. (SKIPPED)",
      },
    },
  },
  // Test skipped - fails in CI
  // play: async ({ canvasElement }) => {
  //   const canvas = within(canvasElement);
  //
  //   await waitFor(() => {
...
export const ErrorTest: Story = {
  parameters: {
    docs: {
      description: {
        story:
          'Test de soumission du formulaire : attend le chargement des iframes Hipay, attend que le SDK soit complètement prêt, puis soumet automatiquement le formulaire vide. Hipay devrait retourner des erreurs de validation sur getPaymentData() qui seront affichées. (SKIPPED)',
      },
    },
  },
  render() {
