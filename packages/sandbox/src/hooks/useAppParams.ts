...
const ParamsSchema = z.object({
  proposalId: z.string().regex(/[0-9]+/).optional(),
  bookingId: z.string().regex(/[0-9]+/).optional(),
...
export function useAppParams() {
  ...
  if (match) {
    const customerId = auth.isAuthenticated && result?.issuer.toUpperCase() === OidcIssuerTypes.GM ? (auth?.user?.profile?.sub as string) : customer_id;
